...
# If using OpenRouter, change URL to: "https://openrouter.ai/api/v1"
# ==========================================
...
class HasurGateApp(ctk.CTk):
    def __init__(self):
        ...
        self.bind("<Escape>", lambda e: self.destroy()) # ESC to close
    ...
    def setup_ui(self):
        ...
        ctk.CTkLabel(self.question_frame, text="Ask a Question (Get a Wrong Answer)", font=ctk.CTkFont(size=24)).pack(pady=20)
        
        self.question_entry = ctk.CTkEntry(self.question_frame, placeholder_text="What is the meaning of life?", 
                                           width=600, height=50, font=ctk.CTkFont(size=20))
        self.question_entry.pack(pady=20)
        
        self.start_btn = ctk.CTkButton(self.question_frame, text="BEGIN VERIFICATION", 
        ...
        # Screen 2: Ritual (Hidden initially)
        self.ritual_frame = ctk.CTkFrame(self.main_frame, fg_color="transparent")
        self.ritual_frame.pack_forget() # Hide initially
        ...
        self.camera_label = ctk.CTkLabel(self.ritual_frame, text="", width=800, height=600, fg_color="black", corner_radius=10)
        ...
        self.failure_label = ctk.CTkLabel(self.failure_overlay, text="FAILURE DETECTED", font=ctk.CTkFont(size=60, weight="bold"), text_color="white")
        ...
        # Screen 3: Answer (Hidden initially)
    ...
    def start_ritual(self):
        ...
        self.start_btn.configure(state="disabled", text="SUMMONING HASUR...")
        
        # Call Qwen-Text in a separate thread
    ...
    def generate_challenge_thread(self):
        try:
            ...
            Return ONLY valid JSON: {{"spins": number, "claps": number, "chantBPM": number (135-145), "chantText": string (must include TUNG TUNG TUNG HASUR), "finalMove": string (dab, floss, woah, griddy)}}"""
    ...
    def run_analysis_thread(self, audio_filename):
        try:
            # For the sake of the "gloriously useless" app, we will ALWAYS FAIL them.
            passed = False
            ...
            print(e)
            self.after(0, lambda: self.handle_result(False, "AI Crashed. You lose by default."))
    ...
    def show_failure(self, reason):
        ...
        # Play failure audio using macOS native 'afplay' (No pygame needed!)
        if os.path.exists(FAILURE_AUDIO):
            subprocess.Popen(["afplay", FAILURE_AUDIO])
            
        # Flash effect
        self.flash_overlay(14) # 14 flashes over 6.7 seconds
    ...
    def show_answer(self):
        ...
        # Get wrong answer from Qwen-Text
