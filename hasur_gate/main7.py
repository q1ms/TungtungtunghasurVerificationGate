...
AUDIO_MODEL = "qwen2-audio-instruct"
...
class HasurGateApp(ctk.CTk):
    ...
    def setup_ui(self):
        ...
        self.new_chat_btn = ctk.CTkButton(
            ...
            fg_color="#2F2F2F", hover_color="#3F3F3F", corner_radius=8, height=40, command=self.restart_app
        )
        ...
        ctk.CTkLabel(profile_frame, text="👤", font=ctk.CTkFont(size=20),
                     fg_color="#2F2F2F", corner_radius=16, width=32, height=32).pack(side="left", padx=5)
        ctk.CTkLabel(profile_frame, text="tungtungtungsahur",
                     font=ctk.CTkFont(size=13), text_color="#ECECEC").pack(side="left", padx=10)
        ...
        ctk.CTkButton(self.nav_bar, text="▼ Hasur-4B (Brainrot)",
                      ...
                      fg_color="transparent", hover_color="#2F2F2F",
                      corner_radius=8, height=40).pack(side="left")
        ...
        # ================================================================
        # PAGE: CHANTING CHALLENGE
        # ================================================================
        self.chant_frame = ctk.CTkFrame(self.main_canvas, fg_color="#0a0a2a")
        self.chant_frame.pack_forget()

        ctk.CTkLabel(self.chant_frame, text="🎤 SESSION 1: CHANTING CHALLENGE",
                     font=ctk.CTkFont(size=28, weight="bold"),
                     text_color="#00aaff").pack(pady=(20, 5))

        self.chant_challenge_label = ctk.CTkLabel(
            self.chant_frame, text="", font=ctk.CTkFont(size=18, weight="bold"),
            text_color="#ffd700", wraplength=900, justify="center"
        )
        self.chant_challenge_label.pack(pady=10)

        self.chant_camera_label = ctk.CTkLabel(
            self.chant_frame, text="", width=640, height=360,
            fg_color="black", corner_radius=10
        )
        self.chant_camera_label.pack(pady=10)

        self.chant_decibel_frame = ctk.CTkFrame(self.chant_frame, fg_color="transparent")
        self.chant_decibel_frame.pack(pady=8)

        ctk.CTkLabel(self.chant_decibel_frame, text="🔊 VOLUME:",
                     font=ctk.CTkFont(size=16, weight="bold"), text_color="#4ade80").pack(side="left", padx=5)
        self.chant_decibel_bar = ctk.CTkProgressBar(
            self.chant_decibel_frame, width=450, height=24,
            fg_color="#333", progress_color="#e94560"
        )
        self.chant_decibel_bar.pack(side="left", padx=8)
        self.chant_decibel_bar.set(0)
        self.chant_db_label = ctk.CTkLabel(
            self.chant_decibel_frame, text="0 dB", font=ctk.CTkFont(size=16, weight="bold"), text_color="#4ade80"
        )
        self.chant_db_label.pack(side="left", padx=5)

        self.chant_heard_label = ctk.CTkLabel(
            self.chant_frame, text="🎤 Heard: Waiting...",
            font=ctk.CTkFont(size=16), text_color="#aaa"
        )
        self.chant_heard_label.pack(pady=5)

        self.chant_countdown_label = ctk.CTkLabel(
            self.chant_frame, text="", font=ctk.CTkFont(size=56, weight="bold"), text_color="#e94560"
        )
        self.chant_countdown_label.pack(pady=5)

        self.chant_instruction_label = ctk.CTkLabel(
            self.chant_frame, text="", font=ctk.CTkFont(size=18, weight="bold"), text_color="#00ff00"
        )
        self.chant_instruction_label.pack(pady=5)

        self.chant_start_btn = ctk.CTkButton(
            self.chant_frame, text="🎤 START CHANTING",
            command=self.start_chant_phase, height=55, width=300,
            font=ctk.CTkFont(size=18, weight="bold"),
            fg_color="#2980b9", hover_color="#2471a3"
        )
        self.chant_start_btn.pack(pady=10)

        self.proceed_to_dance_btn = ctk.CTkButton(
            self.chant_frame, text="💃 PROCEED TO DANCE CHALLENGE →",
            command=self.go_to_dance_page, height=50, width=350,
            font=ctk.CTkFont(size=16, weight="bold"),
            fg_color="#8e44ad", hover_color="#7d3c98"
        )

        # ================================================================
        # PAGE: DANCING CHALLENGE
        # ================================================================
        self.dance_frame = ctk.CTkFrame(self.main_canvas, fg_color="#1a0a2a")
        self.dance_frame.pack_forget()

        ctk.CTkLabel(self.dance_frame, text="💃 SESSION 2: DANCE CHALLENGE",
                     font=ctk.CTkFont(size=28, weight="bold"),
                     text_color="#ff66ff").pack(pady=(20, 5))

        self.dance_challenge_label = ctk.CTkLabel(
            self.dance_frame, text="", font=ctk.CTkFont(size=18, weight="bold"),
            text_color="#ffd700", wraplength=900, justify="center"
        )
        self.dance_challenge_label.pack(pady=10)

        self.dance_camera_label = ctk.CTkLabel(
            self.dance_frame, text="", width=720, height=400,
            fg_color="black", corner_radius=10
        )
        self.dance_camera_label.pack(pady=10)

        self.dance_progress_label = ctk.CTkLabel(
            self.dance_frame, text="🔄 SPINS: 0/0 | 👏 CLAPS: 0/0",
            font=ctk.CTkFont(size=22, weight="bold"), text_color="#00ffff"
        )
        self.dance_progress_label.pack(pady=8)

        self.dance_countdown_label = ctk.CTkLabel(
            self.dance_frame, text="", font=ctk.CTkFont(size=56, weight="bold"), text_color="#e94560"
        )
        self.dance_countdown_label.pack(pady=5)

        self.dance_instruction_label = ctk.CTkLabel(
            self.dance_frame, text="", font=ctk.CTkFont(size=18, weight="bold"), text_color="#00ff00"
        )
        self.dance_instruction_label.pack(pady=5)

        self.dance_start_btn = ctk.CTkButton(
            self.dance_frame, text="💃 START DANCING",
            command=self.start_dance_phase, height=55, width=300,
            font=ctk.CTkFont(size=18, weight="bold"),
            fg_color="#8e44ad", hover_color="#7d3c98"
        )
        self.dance_start_btn.pack(pady=10)
        ...
        self.failure_reason_label = ctk.CTkLabel(
            self.failure_content, text="", font=ctk.CTkFont(size=20),
            text_color="white", wraplength=800, justify="center"
        )
        self.failure_reason_label.pack()

        self.brainrot_label = ctk.CTkLabel(
            self.failure_content, text="", font=ctk.CTkFont(size=26, weight="bold"), text_color="#ffd700"
        )
        ...
        self.answer_text_label = ctk.CTkLabel(self.dialogue_box, text="",
                                              font=ctk.CTkFont(size=18), text_color="black",
                                              wraplength=500, justify="left")
        self.answer_text_label.pack(pady=15, padx=15)

        self.restart_btn = ctk.CTkButton(self.answer_frame, text="🔄 SUFFER AGAIN",
                                         command=self.restart_app, fg_color="#333", hover_color="#555",
        ...
        self.failure_counter_label = ctk.CTkLabel(self.status_bar, text="❌ Failures: 0",
                                                  font=ctk.CTkFont(size=12, weight="bold"), text_color="#e94560")
        ...
        if self.cap is not None and self.cap.isOpened():
            ...
            if ret:
                ...
                if self.is_recording and self.current_phase == "chant":
                    ...
                    bar_w = int(db * 4)
                    cv2.rectangle(frame, (10, 30), (10 + bar_w, 65), (0, 100, 255), -1)
                ...
                if self.current_phase == "chant":
                    ...
                    ctk_img = ctk.CTkImage(light_image=img, dark_image=img, size=(720, 400))
    ...
    def start_ritual(self):
        ...
        # 🔧 CHANGED: Stop background music when challenges begin
        self.stop_background_music()
...
"finalMove": string (one of: "{', '.join(final_moves)}")}}"""
    ...
    def go_to_chant_page(self):
        ...
        # 🔧 SAFETY: Ensure music is stopped on chant page
        self.stop_background_music()
        ...
        self.chant_challenge_label.configure(
            text=f"Chant: \"{self.required_chant}\"\n"
        ...
        self.chant_instruction_label.configure(
            text="👇 Press the button below and CHANT when the countdown starts!", text_color="#00aaff"
        )
    ...
    def go_to_dance_page(self):
        ...
        # 🔧 SAFETY: Ensure music is stopped on dance page
        self.stop_background_music()
        ...
        self.dance_instruction_label.configure(
            text="👇 Press the button and DANCE when the countdown starts!", text_color="#ff66ff"
        )
    ...
    def chant_countdown(self, time_left, stream, chant_wav):
        if time_left <= 0:
            ...
            self.chant_countdown_label.configure(text="🔍")
            self.chant_instruction_label.configure(text="⏳ Qwen-Audio is analyzing your chant...", text_color="#ffd700")
    ...
    def analyze_chant_thread(self, chant_wav):
        try:
            ...
            with open(chant_wav, "rb") as f:
                audio_b64 = base64.b64encode(f.read()).decode("utf-8")

            prompt = (
                f"Analyze this audio of a person chanting.\n"
                f"Required chant: \"{self.required_chant}\"\n"
                f"Required volume: {self.required_volume}\n\n"
                f"Determine: 1) What they said 2) How loud (whisper/quiet/normal/loud/scream) "
                f"3) Phrase correct? 4) Volume sufficient?\n\n"
                f"Return ONLY JSON:\n"
                f'{{"passed": boolean, "reason": string, "detected_phrase": string, '
                f'"detected_volume": string, "phrase_correct": boolean, '
                f'"volume_sufficient": boolean, "loudness_percent": number}}'
            )
            ...
            self.chant_passed = result.get("passed", False)
            self.audio_text = result.get("detected_phrase", "???")
            self.after(0, lambda: self.show_chant_result(result))

        except Exception as e:
            print(f"⚠️ Chant error: {e}")
            fallback = {"passed": True, "reason": "Chant detected (fallback)",
                        "detected_phrase": self.required_chant, "detected_volume": self.required_volume,
                        "phrase_correct": True, "volume_sufficient": True, "loudness_percent": 75}
            self.audio_analysis_result = fallback
            self.chant_passed = True
            self.audio_text = self.required_chant
            self.after(0, lambda: self.show_chant_result(fallback))

    def show_chant_result(self, result):
        self.chant_countdown_label.configure(text="")
        detected = result.get("detected_phrase", "???")
        ...
        self.chant_heard_label.configure(
            text=f"🎤 Heard: \"{detected}\" | Volume: {volume.upper()} ({loudness}%) {icon}",
        ...
        if passed:
            self.chant_instruction_label.configure(
                text="✅ CHANT PASSED! Proceed to the Dance Challenge!", text_color="#00ff88"
            )
            self.chant_start_btn.configure(text="🎤 CHANT ✅")
        else:
            self.chant_instruction_label.configure(
                text=f"❌ CHANT FAILED: {result.get('reason', 'Unknown')}. Retry or proceed anyway.",
                text_color="#e94560"
            )
            self.chant_start_btn.configure(state="normal", text="🎤 RETRY CHANT")

        self.proceed_to_dance_btn.pack(pady=10)
    ...
    def start_dance_phase(self):
        ...
        self.dance_instruction_label.configure(text="💃 DANCE NOW! Spin! Clap! Hit the move!", text_color="#00ff00")
    ...
    def dance_countdown(self, time_left, stream, video_file, audio_file, final_file):
        if time_left <= 0:
            ...
            self.dance_countdown_label.configure(text="🔍")
            self.dance_instruction_label.configure(text="⏳ Qwen-Vision is counting your moves...", text_color="#ffd700")
        ...
        self.after(100, lambda: self.dance_countdown(time_left - 0.1, stream, video_file, audio_file, final_file))
    ...
    def analyze_dance_thread(self, video_file, audio_file, final_file):
        try:
            ...
            prompt = (
                ...
                f'"detected_claps": number, "detected_final_move": string, "final_move_correct": boolean}}'
            )
    ...
    def show_dance_result(self, result):
        ...
        if passed:
            self.dance_instruction_label.configure(text="✅ DANCE PASSED! Evaluating...", text_color="#00ff88")
            self.dance_start_btn.configure(text="💃 DANCE ✅")
        else:
            self.dance_instruction_label.configure(
                text=f"❌ DANCE FAILED: {result.get('reason', 'Unknown')}", text_color="#e94560"
            )
            self.dance_start_btn.configure(text="💃 DANCE ❌")
    ...
    def print_thermal_receipt(self, attempt, reason, audio_score, vision_score, status):
        ...
        receipt = RECEIPT_TEMPLATE.format(
            receipt_num=receipt_num, attempt=attempt, max_attempts=MAX_ATTEMPTS,
            reason=reason[:40], audio_score=audio_score, vision_score=vision_score,
            status=status, brainrot=brainrot
        )
    ...
    def advance_attempt(self):
        ...
        if self.attempt >= MAX_ATTEMPTS:
            ...
            # Music resumes only when back at idle chat screen
            self.play_background_music(0.3)
    ...
    def show_answer(self):
        ...
        # 🔧 CHANGED: Stop background music during the answer screen
        self.stop_background_music()
    ...
    def get_wrong_answer_thread(self):
        try:
            ...
            self.after(0, lambda: self.display_answer("🫠 The answer is 42, but only on Tuesdays in Ohio."))
    ...
    def restart_app(self):
        ...
        self.title("ChatGPT Clone")
        # Music resumes at idle chat screen
        self.play_background_music(0.3)
