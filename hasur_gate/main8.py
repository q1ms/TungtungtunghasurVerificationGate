...
class HasurGateApp(ctk.CTk):
    ...
    def setup_ui(self):
        ...
        # PROCEED button — navigates to dance page when clicked
    ...
    def start_ritual(self):
        ...
        # Stop background music when challenges begin
        self.stop_background_music()
...
"finalMove": string (one of: "{', '.join(final_moves)}")}}"""
    ...
    def show_chant_result(self, result):
        ...
        if passed:
            self.chant_instruction_label.configure(
                text="✅ CHANT PASSED! Click below to proceed to Dance!", text_color="#00ff88"
            )
            self.chant_start_btn.configure(text="🎤 CHANT ✅")
        else:
            self.chant_instruction_label.configure(
                text=f"❌ CHANT FAILED: {result.get('reason', 'Unknown')}. Click below to proceed anyway.",
                text_color="#e94560"
            )
            self.chant_start_btn.configure(text="🎤 CHANT ❌")

        # Show the PROCEED button — clicking it navigates to the dance page
