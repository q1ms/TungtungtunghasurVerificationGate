...
# ==========================================
# >>> INSERT YOUR MODEL SCOPE API KEY HERE <<<
# ModelScope token starts with "ms-"
# ==========================================
API_KEY = "ms-1f6f349d-a735-4d78-be10-9a0d3a4fc9e4"  # <--- REPLACE WITH YOUR MODEL SCOPE TOKEN
...
class HasurGateApp(ctk.CTk):
    ...
    def setup_ui(self):
        ...
        # Header
        header_row = ctk.CTkFrame(sidebar_inner, fg_color="transparent")
        ...
        self.new_chat_btn.pack(fill="x")

        # Model selector
        model_frame = ctk.CTkFrame(sidebar_inner, fg_color="transparent")
        model_frame.pack(fill="x", pady=(10, 20))

        self.model_btn = ctk.CTkButton(
            model_frame,
            text="⚡ Hasur-4B (Brainrot)",
        ...
        for item in chat_history[:4]:
            ...
            chat_item.pack(fill="x", pady=1)

        # Bottom profile
        bottom_spacer = ctk.CTkFrame(sidebar_inner, fg_color="transparent")
        bottom_spacer.pack(fill="both", expand=True)

        upgrade_frame = ctk.CTkFrame(
            sidebar_inner,
            fg_color="#2F2F2F",
            corner_radius=8,
            height=50
        ...
        ctk.CTkLabel(
            profile_frame,
            text="tungtungtungsahur",
            font=ctk.CTkFont(size=13),
            text_color="#ECECEC"
        ).pack(side="left", padx=10)

        # ===== RIGHT COLUMN (Main Canvas) =====
        self.main_canvas = ctk.CTkFrame(
            self.main_container,
            fg_color="#212121"
        )
        ...
        for i, suggestion in enumerate(suggestions):
            ...
            chip.configure(command=lambda s=suggestion: self.question_entry.insert(0, s))
        ...
        self.question_entry = ctk.CTkEntry(
            input_row,
            placeholder_text="Message Hasur...",
        ...
        ctk.CTkLabel(
            self.input_frame,
            text="Hasur can make mistakes. Check important info.",
        ...
        # ==========================================
        self.brainrot_frame = ctk.CTkFrame(self.main_canvas, fg_color="#0a0a1a")
        self.brainrot_frame.pack_forget()

        # --- PHASE 1: VOICE CHALLENGE ---
        self.voice_phase_frame = ctk.CTkFrame(self.brainrot_frame, fg_color="transparent")
        # Packed dynamically

        self.voice_challenge_label = ctk.CTkLabel(
            self.voice_phase_frame, text="", font=ctk.CTkFont(size=28, weight="bold"), text_color="#00ffff", wraplength=1000
        )
        self.voice_challenge_label.pack(pady=20)

        self.start_chant_btn = ctk.CTkButton(
            self.voice_phase_frame, text="🎤 START CHANTING", command=self.start_voice_challenge,
            font=ctk.CTkFont(size=24, weight="bold"), height=60, width=300, fg_color="#e94560", hover_color="#c73e54"
        )
        self.start_chant_btn.pack(pady=20)

        self.voice_progress_label = ctk.CTkLabel(
            self.voice_phase_frame, text="🎤 Chant: Waiting for you to speak...", font=ctk.CTkFont(size=22, weight="bold"), text_color="#4ade80", wraplength=800
        )
        self.voice_progress_label.pack(pady=10)

        self.db_meter_label = ctk.CTkLabel(
            self.voice_phase_frame, text="🔊 Loudness: 0.0 dB", font=ctk.CTkFont(size=18, weight="bold"), text_color="#00ffff"
        )
        self.db_meter_label.pack(pady=(10, 0))

        self.db_progress_bar = ctk.CTkProgressBar(self.voice_phase_frame, width=500, height=25, corner_radius=10)
        self.db_progress_bar.pack(pady=5)
        self.db_progress_bar.set(0)

        self.voice_countdown_label = ctk.CTkLabel(
            self.voice_phase_frame, text="⏳ 0.0s", font=ctk.CTkFont(size=60, weight="bold"), text_color="#e94560"
        )
        self.voice_countdown_label.pack(pady=20)

        # --- PHASE 2: MOVEMENT CHALLENGE ---
        self.move_phase_frame = ctk.CTkFrame(self.brainrot_frame, fg_color="transparent")
        # Packed dynamically

        self.move_challenge_label = ctk.CTkLabel(
            self.move_phase_frame, text="", font=ctk.CTkFont(size=24, weight="bold"), text_color="#ffd700", wraplength=1000
        )
        self.move_challenge_label.pack(pady=15)

        self.camera_label = ctk.CTkLabel(
            self.move_phase_frame, text="", width=800, height=500, fg_color="black", corner_radius=10
        )
        self.camera_label.pack(pady=10)

        self.progress_label = ctk.CTkLabel(
            self.move_phase_frame, text="🔄 SPINS: 0/0 | 👏 CLAPS: 0/0", font=ctk.CTkFont(size=22, weight="bold"), text_color="#00ffff"
        )
        self.progress_label.pack(pady=10)

        self.move_countdown_label = ctk.CTkLabel(
            self.move_phase_frame, text="⏳ 0.0s", font=ctk.CTkFont(size=60, weight="bold"), text_color="#e94560"
        )
        self.move_countdown_label.pack(pady=10)
        
        self.instruction_label = ctk.CTkLabel(
            self.move_phase_frame, text="", font=ctk.CTkFont(size=24, weight="bold"), text_color="#00ff00"
        )
        ...
        if self.cap is not None and self.cap.isOpened():
            ...
            if ret:
                ...
                if self.is_recording:
                    ...
                    if self.audio_text:
                        cv2.putText(frame, f"CHANT: {self.audio_text[:25]}", 
    ...
    def show_ritual_screen(self):
        ...
        # Show Phase 1 UI, Hide Phase 2 UI
        self.voice_phase_frame.pack(fill="both", expand=True)
        self.move_phase_frame.pack_forget()
        
        voice_text = (
            f"⚔️ PHASE 1: VOICE CHALLENGE ⚔️\n"
            f"{warning} ATTEMPT {self.attempt} {warning}\n\n"
            f"🗣️ CHANT: '{self.challenge.get('chantText', 'TUNG TUNG TUNG HASUR')}'\n"
            f"BPM: {self.challenge.get('chantBPM', 140)} | HYPE: {self.challenge.get('hypeLevel', 7)}/10\n\n"
            f"Press the button below when you are ready to chant!"
        )
        self.voice_challenge_label.configure(text=voice_text)
        self.start_chant_btn.pack(pady=20) # Ensure it's visible
        
        # Reset Voice UI
        self.voice_progress_label.configure(text="🎤 Chant: Waiting for you to speak...")
        self.db_progress_bar.set(0)
        self.db_meter_label.configure(text="🔊 Loudness: 0.0 dB")
        self.voice_countdown_label.configure(text="⏳ 0.0s")
        
        # Reset Movement UI for later
    ...
    def start_voice_challenge(self):
        self.start_chant_btn.pack_forget() # Hide button
        self.voice_countdown_label.configure(text="⏳ 5.0s") # 5 seconds for chanting
        
        # Setup Audio Recording
        samplerate = 44100
        self.audio_data = []
        
        def voice_audio_callback(indata, frames, time, status):
            ...
            if len(indata) > 0:
                ...
                if rms > 0:
                    ...
                    db = -60.0
                scaled_db = max(0.0, min(1.0, (db + 60.0) / 50.0))
                self.audio_loudness = scaled_db * 100
                
                # Simulate live transcription
                target_chant = self.challenge.get('chantText', 'TUNG TUNG TUNG HASUR')
                self.after(0, lambda t=target_chant: self.voice_progress_label.configure(
                    text=f"🎤 AI Hearing: {t.upper()}..."
                ))
                self.after(0, lambda d=scaled_db, val=db: self.update_db_meter(d, val))

        self.voice_stream = sd.InputStream(samplerate=samplerate, channels=1, callback=voice_audio_callback)
        self.voice_stream.start()
        
        self.voice_countdown = 5.0
        self.update_voice_countdown(5.0)

    def update_db_meter(self, scaled_db, db_val):
        self.db_progress_bar.set(scaled_db)
        if scaled_db > 0.7: color = "#ff0000"
        elif scaled_db > 0.4: color = "#ffd700"
        else: color = "#00ffff"
        self.db_progress_bar.configure(progress_color=color)
        self.db_meter_label.configure(text=f"🔊 Loudness: {db_val:.1f} dB")

    def update_voice_countdown(self, time_left):
        if time_left <= 0:
            ...
            self.voice_progress_label.configure(text="✅ Chant Recorded! Preparing Movement Phase...")
            self.after(1500, self.start_movement_challenge) # Short delay before Phase 2
            return
            
        self.voice_countdown_label.configure(text=f"⏳ {time_left:.1f}s")
        self.after(100, lambda: self.update_voice_countdown(time_left - 0.1))

    def start_movement_challenge(self):
        # Hide Phase 1, Show Phase 2
        self.voice_phase_frame.pack_forget()
        self.move_phase_frame.pack(fill="both", expand=True)
        
        warning = "🔥" * min(self.attempt, 5)
        move_text = (
            f"⚔️ PHASE 2: MOVEMENT CHALLENGE ⚔️\n"
            f"{warning} ATTEMPT {self.attempt} {warning}\n\n"
            f"🏃 ACTION:\n"
        ...
        self.move_challenge_label.configure(text=move_text)
        self.instruction_label.configure(text="🎥 PERFORM THE MOVES NOW! 🎥")
        ...
        self.video_writer = cv2.VideoWriter(self.current_video_filename, fourcc, 30.0, (800, 500))
        
        # Simulate real-time progress counting for the UI
        threading.Thread(target=self.simulate_realtime_progress, args=(duration,), daemon=True).start()
        
        self.move_countdown = duration
        self.update_movement_countdown(duration)

    def simulate_realtime_progress(self, total_time):
        # Simulate the AI "seeing" the spins and claps over time
        steps = 20
        ...
        for i in range(steps):
            if not self.is_recording:
                break
            time.sleep(interval)
            # Randomly increment spins and claps to simulate vision detection
            if self.realtime_spins < self.required_spins and random.random() > 0.4:
                self.realtime_spins += 1
            if self.realtime_claps < self.required_claps and random.random() > 0.5:
                self.realtime_claps += 1
            
            self.after(0, lambda s=self.realtime_spins, c=self.realtime_claps: self.progress_label.configure(
                text=f"🔄 SPINS: {s}/{self.required_spins} | 👏 CLAPS: {c}/{self.required_claps}"
            ))

    def update_movement_countdown(self, time_left):
        if time_left <= 0:
            self.is_recording = False
            ...
            self.move_countdown_label.configure(text="🔍")
            threading.Thread(target=self.run_analysis_thread, daemon=True).start()
            return
            
        self.move_countdown_label.configure(text=f"⏳ {time_left:.1f}s")
        self.after(100, lambda: self.update_movement_countdown(time_left - 0.1))
    ...
    # ==========================================
    # AUDIO ANALYSIS (SIMULATED for stability)
    # ==========================================
    def analyze_chant_audio(self, audio_path):
        print("🎤 Audio analysis simulated (pass).")
        return {
            "transcription": "TUNG TUNG TUNG HASUR",
            "correct_phrase": True,
            "bpm": 140,
            "hype": 8,
            "pronunciation": True,
    ...
    def run_analysis_thread(self):
        try:
            ...
            audio_passed = audio_result.get("passed", True)
            audio_score = min(100, max(0, audio_result.get("hype", 7) * 10 + 30))
            ...
            # Update realtime counters for overlay
